...
def run_nmap_vuln_scan(target_ip: str, ports: str) -> dict:
    ...
    nmap_command = ["sudo", "nmap", "-Pn", "-sT", "-sV", "-f", "-g", "53", "-T5", "--data-length", "24", "--spoof-mac", "0", "-p", ports, "--script", "vuln,smb-vuln*", target_ip]
    try:
        ...
        return {"status": "error", "message": "El escaneo de Nmap superó el tiempo límite (5 minutos) y fue abortado."}
    except FileNotFoundError:
        return {"status": "error", "message": "Nmap no está instalado o no se puede encontrar en el PATH."}
    except subprocess.CalledProcessError as e:
        return {"status": "error", "message": f"Error al ejecutar escaneo de vulnerabilidades Nmap: {e.stderr}"}