...
from config.settings import TARGET_IP, TARGET_PORTS  # Cambiado RED_SUBNET por TARGET_IP
...
def ejecutar_ataque(modo="normal") -> dict:
    ...
    try:
        if modo == "vuln":
            resultado = run_nmap_vuln_scan(TARGET_IP, TARGET_PORTS) # Ahora ataca la IP exacta
        else:
            resultado = run_nmap_scan(TARGET_IP, TARGET_PORTS) # Ahora ataca la IP exacta
